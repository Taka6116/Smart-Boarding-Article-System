...
/**
 * generateContent を順に試すモデル（429 は同一モデルで待機再試行、その他は次へ）。
 * `-latest` エイリアスは v1beta で 404 になることがあるため固定名のみ使う。
 */
const GEMINI_MODELS = ['gemini-2.5-flash', 'gemini-2.0-flash', 'gemini-1.5-flash', 'gemini-1.5-pro'] as const
...
async function generateContentWithFallback(apiKey: string, prompt: string): Promise<string> {
  ...
  console.error('[Gemini] 全候補モデルで失敗', { models: [...GEMINI_MODELS], lastDetail })
  ...
  const hint = quotaLike
    ...
    : 'モデル廃止・API変更・認証の問題の可能性があります。GEMINI_API_KEY と利用可能モデル（AI Studio の ListModels）を確認してください。'
